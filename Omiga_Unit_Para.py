...
# 下一步,自动将每一个设备写成一个类.
# 通过ProcessName.UnitName.Parameter_Value的形式调用.

# Load Package.
import re
import numpy as np

# 要读取的Unit文件,从OmegaLand导出.
# 此处的Test_DB.txt为例子.
File = 'Unit Variable.txt'

# 生成类的新文件New_File
New_File = 'Unit_Variable_Class.py'
        ...
        for s in f:
            ...
            # 根据参数行前面不同的符号,分为三种情况.
                ...
                if len(Genepara):
                    ...
                    Genepara_Name = Genepara_list[0].replace('*','_')
                    Genepara_Value = Genepara_list[1]
                    Genepara_Notes = ' '.join(Genepara_list[2:])

                    # 写入General Parameters
                    New_s = (
                        '\t\t# General Parameters\n'
                        '\t\tself.' + Genepara_Name + '_Value = ' + '\"' + Genepara_Value  + '\"' + '\n'
                        '\t\tself.' + Genepara_Name + '_Notes = ' + '\"' + Genepara_Notes + '\"' + '\n'
                        '\n'
                   )
                    New_f.write(New_s)
                ...
                if len(Pripara):
                    ...
                    # 写入Primary Parameters
                    New_s = (
                        '\t\t# Primary Parameters\n'
                        '\t\tself.' + Pripara_Name + '_Value = ' + '\"' + Pripara_Value + '\"' + '\n'
                        '\t\tself.' + Pripara_Name + '_Notes = ' + '\"' + Pripara_Notes + '\"' + '\n'
                        '\n'
                   )
                    New_f.write(New_s)
                ...
                try:
                    ...
                    if Line_Num > Priparalist_Value_Start & Line_Num < Priparalist_Value_End + 1:
                        ...
                        #      所对应的Value为程序内容,
                        if k == 1:
                            Priparalist_Value.append(_Priparalist_Value[0])

                            #   如果此时所在行已经到Priparalist_Value的最后一行,
                            #   则整理获得最终的Priparalist_Value.
                            if Line_Num == Priparalist_Value_End:
                                Priparalist_Value = '\t'.join(Priparalist_Value)

                                # 写入Primary Parameters List Value
                                New_s = (
                                    '\t\t# Primary Parameters List Value\n'
                                    '\t\tself.' + Priparalist_Name + '_List' + '_Value = '  + '\"' + Priparalist_Value + '\"\n'
                               )
                                New_f.write(New_s)
                        ...
                        if k == 2:
                            ...
                            #   此时获得最终的Pripara_Value
                                if len(Priparalist_Value) == Priparalist_Value_Lines * Priparalist_Value_Columns:
                                    ...
                                    New_s = (
                                        '\t\t# Primary Parameters List Value\n'
                                        '\t\tself.' + Priparalist_Name + '_List' + '_Value = ' + np.array_repr(Priparalist_Value) + '\n'
                                   )
                                    New_f.write(New_s)
